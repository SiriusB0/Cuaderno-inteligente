# Script para arreglar el código automáticamente
...
# Backup
$content | Out-File "js\views\StudyView_BACKUP.js" -Encoding UTF8

# Función insertCodeBlock nueva
$newInsertCode = @'
insertCodeBlock() {
...
# Reemplazar insertCodeBlock (buscar desde "insertCodeBlock() {" hasta el próximo "    }" que cierra la función)
$pattern1 = '(?s)(insertCodeBlock\(\) \{).*?(\n    \})'
$content = $content -replace $pattern1, "$newInsertCode`n    }"

# Función injectWorkingCodeStyles nueva
$newStylesFunc = @'
...
# Reemplazar injectHighlightStyles con injectWorkingCodeStyles
$pattern2 = '(?s)(injectHighlightStyles\(\) \{).*?(\n    \})'
$content = $content -replace $pattern2, "$newStylesFunc`n    }"

# Guardar
$content | Out-File $file -Encoding UTF8 -NoNewline

Write-Host "Archivo actualizado correctamente"
Write-Host "Backup guardado en: js\views\StudyView_BACKUP.js"
